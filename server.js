...
var rollbar = new Rollbar({
  accessToken: "a34fd0b9b1a4465d9e1e93a013b731f1",
  captureUncaught: true,
  captureUnhandledRejections: true,
});

app.get("/", (req, res) => {
  rollbar.log("html rendered successfully");
  res.sendFile(path.join(__dirname, "/public/index.html"));
});

app.use(express.static(path.join(__dirname, "/public")));
// app.use("/styles", path.join(__dirname, "/public/index.css"));
// app.use("/js", path.join("/public/index.js"));

app.get("/api/robots", (req, res) => {
  try {
    res.status(200).send(botsArr);
  } catch (error) {
    rollbar.error("Couldn't get bots array");
